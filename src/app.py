...
</style></head><body><header><div class="eyebrow">Local reference implementation</div><h1>Turn inbound messages into an actionable CRM queue.</h1><p class="intro">Structured intake, deterministic or optional AI analysis, duplicate detection, safe local storage, follow-up priorities, and CSV handoff.</p></header>
...
def main() -> None:
    ...
    try:
        ...
        raise
    print(f"CRM reference app running at http://{settings.host}:{settings.port}")
    try:
